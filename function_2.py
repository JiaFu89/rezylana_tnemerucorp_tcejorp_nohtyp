        ...
        for agency in agency_to_procurements:
            ...
            with open(file_path, 'w') as f: #Open the txt file as writable file
                for procurement in agency_to_procurements[agency]:
                    for attr in procurement.ATTRIBUTES:
                        f.write('%s: %s\n' % (attr, str(getattr(procurement, attr)))) #Write the government procurement details into the text file

                    f.write('='*1000 + '\n') #Divider to separate different tender number
